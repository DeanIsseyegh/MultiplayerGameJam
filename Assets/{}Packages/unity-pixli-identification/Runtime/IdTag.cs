...
public partial struct IdTag : IEquatable<IdTag>
{
	...
	{
		this._tag = tag.ToValidClassName();
	...
	public override string ToString() => $"Tag Display Value: {this._tag.ToDisplayValue()} | Tag: {this._tag} | Tag Id: {this._id}";
	...
	public class IdTagPropertyDrawer : MultiSupportPropertyDrawer
	{
		...
		public override void MainDrawGUI(Rect position, SerializedProperty serializedProperty, GUIContent label)
		{
			...
			IdTag idTag = new IdTag(this._tagProperty.stringValue.ToValidClassName());

			//Debug.Log($"IdTag index in pop up: {idTagsContainer.FindIndex(idTag, 0)}");

			//Debug.Log("Tag string value: " + this._tagProperty.stringValue.ToValidClassName());
