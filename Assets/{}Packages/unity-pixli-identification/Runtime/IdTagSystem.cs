...
public class IdTagSystem : ScriptableObjectSingleton<IdTagSystem>
{
	...
	internal void Initialize()
	{
		...
		for (int a = 0; a < this._idTagsContainers.Count; a++)
		{
			...
			{
				// This is done because _tag property is edited directly in the editor, so if it contains spaces then it will contain spaces and nothing is preventing it.
				// But in constructor spaces are removed and we need to remove spaces for it to work with code, but we still want to have kind of beautiful look in inspector.
				// I am not sure yet if this is a good solution because with spaces and without spaces have different hash codes and it may create confusion or invisible bug if someone uses both for the same id.
				// Like instead of `new IdTag(tag: "HelloWorld")` they would write `new IdTag(tag: "Hello World")` and expect it to behave the same way because in the editor it looks like this.
				// But I am preventing this in constructor, so this problem won't occur. The problem would be when someone expects `new IdTag(tag: "HelloWorld")` and `new IdTag(tag: "Hello World")` to be different, while in code they are made to be the same.
				idTags[b] = new IdTag(tag: idTags[b].Tag);
				...
				this._allIdTagsRelativePaths[allIdTagsIndex++] = $"{this._idTagsContainers[a].name}/{idTags[b].Tag.ToDisplayValue()}";
			}
		...
		for (int a = 0; a < this._combinations.Count; a++)
		{
			if (this._combinations[a].Unique)
			{
				...
				this._mixedIdTagsRelativePaths[a] = $"{this._combinations[a].IdTagsContainer.name}/{this._combinations[a].IdTag.Tag.ToDisplayValue()}";
			}
			else
			{
	...
	private void GenerateIdTagsStaticAccess()
	{
		...
		using (StreamWriter streamWriter = new StreamWriter(Path.Combine(PathUtility.ASSETS_PATH_NAME, PathUtility.GetScriptFileDirectoryPath(), "IdTagStaticAccess.cs")))
		{
...
{{1}}	}}}}";
	...
	public class IdTagSystemEditor : MultiSupportEditor
	{
		...
		public override void MainDrawGUI()
		{
			...
			Rect verticalMainRect = EditorGUILayout.BeginVertical(GUIStyle.none); // style
